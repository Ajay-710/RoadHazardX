    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (data.prediction) {
                ...
                if (data.status === true) {
                    // Success Match
                    onSubmit(hazardType, photoPreview, data.confidence, aiStatus, readableAddress, jurisdiction);
                } else {
                    // Mismatch Detected (Step 8 Logic)
                    const msg = `AI processing: You selected "${hazardType}" and AI predicted "${data.prediction}". Authority: ${jurisdiction.Authority}. Are you sure?`;
